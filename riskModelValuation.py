...
def ori_to_val_dist(grid, p, type, interpolation_kind='linear', bins=100, slow=True):
    ...
    # # This solves the problem of stimulusNoise having signal in one quadrant but not in others, (the mean value signal should be taken)
...
def value_efficient_encoding(theta0, sigma_stim, sigma_rep, type, interpolation_kind='linear', bins=100, slow=True):
...
# Take input orientation and gives the decoded distribution
def value_bayesian_decoding(theta0, sigma_stim, sigma_rep, type, interpolation_kind='linear', bins=100, slow=True):
